// Graphic_09.cpp : Defines the entry point for the console application.
//
...
typedef struct bpatch { // struktura dla wielomiana fragmentu Beziera 3 stopnia
	POINT_3D anchors[4][10]; // siatka 4x
...
GLuint genBezier(BEZIER_PATCH patch, int divs) {
	...
	temp[0] = patch.anchors[0][9];
	temp[1] = patch.anchors[1][9];
	temp[2] = patch.anchors[2][9];
	temp[3] = patch.anchors[3][9];
	for (v = 0; v <= divs; v++) {
	...
	for (u = 1; u <= divs; u++) {
		...
		temp[8] = Bernstein(py, patch.anchors[8]);
		temp[9] = Bernstein(py, patch.anchors[9]);
